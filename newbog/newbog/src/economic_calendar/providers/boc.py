...
DEFAULT_BOC_MAPPINGS: dict[str, BOCReleaseMapping] = {
    "BoC Interest Rate Decision": BOCReleaseMapping("BOC_RATE", "central_bank"),
    "Canada CPI": BOCReleaseMapping("CA_CPI", "inflation"),
    "Canada Manufacturing PMI": BOCReleaseMapping("CA_PMI_MFG", "growth"),
    "Canada Services PMI": BOCReleaseMapping("CA_PMI_SVC", "growth"),
    "Canada Employment Change": BOCReleaseMapping("CA_EMPLOYMENT", "labor"),
}


class BOCProvider(EconomicCalendarProvider):
    """Bank of Canada provider using a static schedule for 2026 demo."""

    name = "boc"

    def __init__(self, release_mappings: dict[str, BOCReleaseMapping] | None = None) -> None:
        self._release_mappings = release_mappings or DEFAULT_BOC_MAPPINGS
    ...
    def _normalize_release(self, item: dict[str, Any]) -> EconomicEvent:
        ...
        country = mapping.country if mapping else "CA"
        currency = mapping.currency if mapping else "CAD"
        ...
        event_id = f"boc-{event_code.lower()}-{scheduled_at.date().isoformat()}"
        ...
        return EconomicEvent(
            ...
            tags=("boc", category, currency.lower()),
            metadata={"source": "Bank of Canada", "url": "https://www.bankofcanada.ca/"},
        )
    ...
    def _build_static_schedule(self) -> list[dict[str, Any]]:
        """Canada next-week economic indicators."""
        return [
            {
                "date": "2026-03-31T13:00:00+00:00",
                "title": "Canada Manufacturing PMI",

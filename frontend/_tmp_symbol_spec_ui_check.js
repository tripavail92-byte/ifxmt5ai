...
(async () => {
  ...
  try {
    ...
    console.log(JSON.stringify({
      phase: 'after-terminal-goto',
      url: page.url(),
      selectCount: await page.locator('select').count(),
      selectStates: await page.locator('select').evaluateAll((elements) => elements.map((el) => ({
        disabled: el.disabled,
        value: el.value,
        optionCount: el.options.length,
      }))),
      body: ((await page.locator('body').textContent()) || '').slice(0, 2000),
    }, null, 2));
    await page.waitForFunction(() => {
      const selects = Array.from(document.querySelectorAll('select'));
      return selects.length >= 2 && !selects[0].disabled && !selects[1].disabled;
    }, { timeout: 60000 });
